...
// Auto-defaults allocation for winner prize distribution (Always perfectly equal Player-based splits)
window.updateWinnerPrizeDistribution = function() {
  ...
  if (mode === 'Solo') {
    ...
    const p1 = Math.floor(prize / 2) + (prize % 2);
    const p2 = prize - p1;
    if (input1) input1.value = p1;
    if (input2) input2.value = p2;
  } else if (mode === 'Squad') {
    ...
    const base = Math.floor(prize / 4);
    const rem = prize % 4;
    const p1 = base + (rem >= 1 ? 1 : 0);
    const p2 = base + (rem >= 2 ? 1 : 0);
    const p3 = base + (rem >= 3 ? 1 : 0);
    const p4 = base;

    if (input1) input1.value = p1;
    if (input2) input2.value = p2;
    if (input3) input3.value = p3;
    if (input4) input4.value = p4;
  }
  
  window.updateLiveMatchCardPreview();
};

// Bind elements to trigger default redistribution splits summing up exactly to winner prize (Equal Player-based)
...
if (crudForm) {
  crudForm.addEventListener('submit', async (e) => {
    ...
    // Re-verify strictly equal player-based allocation
    ...
    if (mode === 'Solo') {
      ...
      duo1st = Math.floor(winnerPrize / 2) + (winnerPrize % 2);
      duo2nd = winnerPrize - duo1st;
    } else if (mode === 'Squad') {
      const base = Math.floor(winnerPrize / 4);
      const rem = winnerPrize % 4;
      squad1st = base + (rem >= 1 ? 1 : 0);
      squad2nd = base + (rem >= 2 ? 1 : 0);
      squad3rd = base + (rem >= 3 ? 1 : 0);
      squad4th = base;
    }
    ...
    try {
      ...
      // Payload construction (Equal player-based distributions saved perfectly)
      const payload = {
        title,
        matchName,
        game,
...
// Load values into modification form (Restores equal player distributions cleanly)
window.editTournament = async function(id) {
  try {
    ...
    if (docSnap.exists()) {
      ...
      // Restore equal player splits cleanly
